...
```js
for (let index in dearGovernerLetters) {
  // log each line
  console.log(index, "-->", dearGovernerLetters[index])
}
```
...
```js
for (let index in dearGovernerLetters) {
  if (dearGovernerLetters[index].stance != "Pro") {
    // Do something in here
    console.log(dearGovernerLetters[index].stance)
  }
}
```
